...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateSelectedCategory = useCallback(async (category: DotCategory) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/me/category`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ selectedCategory: category }), credentials: 'include',
      });
      await handleFetchResponse(response);
    } catch (err: any) {
      console.error("Failed to update user category:", err);
  ...
  const isAdmin = !loading && (user?.role === AccessRole.superadmin || user?.role === AccessRole.owner);


  const value: AuthContextType = {
    user, isAuthenticated, isAdmin, loading, error, pendingEmailVerification, setPendingEmailVerification,
    setUser, register, registerOrganization, login, logout, checkAuthStatus: fetchUserProfile, updateSelectedCategory,
